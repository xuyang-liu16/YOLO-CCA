...
def create_modules(modules_defs: list, img_size):
    ...
    for i, mdef in enumerate(modules_defs):
        ...
        if mdef["type"] == "convolutional":
            ...
            modules.add_module('cbam_module', cbam(output_filters[-1], ratio=int(mdef['ratio']), kernel_size=int(mdef['kernelsize'])))

        elif mdef['type'] == 'ECA':
            modules.add_module('ECA_module', ECA(output_filters[-1]))
...
class Darknet(nn.Module):
    ...
    def forward_once(self, x, verbose=False):
        ...
        if self.training:  # train
            return yolo_out, resnet_out, before_yolo_out
